function initTable() {
    ...
    $.ajax({
        url: '../../productType/list',// 跳转到 action

        data: {
            "productType.typeName": $("#name").val()
        },
        traditional: true,
        type: "post",
        cache: false,
                        ...
                        columnDefs: [// 设置列的属性，此处设置第一列不排序
                            ...
                            },
                            {
                                ...
                                "render": function (data,
                                                    type, row) {
                                    var id = row.typeId;
                                    var html = " <a type='button' class='btn btn-sm btn-info btn-white btn-op-ths' title='编辑' href='addOrUpdateProductType.jsp?id="
                                        + id
...
function delArea(obj) {
    var id = obj.name;
    if (confirm("确定删除吗？")) {
        $.post("../../productType/delete", {"id": id}, function (data) {
            if (data.resultJson.result = 'SUCCESS') {
                alert("操作成功！");
